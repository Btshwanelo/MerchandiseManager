import {
  ...
  type InsertAlert,
  type StockTake,
  type InsertStockTake,
  type StockTakeItem,
  type InsertStockTakeItem,
  type StoreAssignment,
  type InsertStoreAssignment,
  type WorkItem,
  type InsertWorkItem,
  WorkItemStatus,
  AlertStatus,
  ...
  type InsertCompetitorMerchandising,
  type MerchandisingPromotion,
  type InsertMerchandisingPromotion,
  type MerchandisingItem,
  type InsertMerchandisingItem,
  type Base64Image,
  type InsertBase64Image,
} from "@shared/schema";
...
export interface IStorage {
  ...
  getPasswordResetToken(
    ...
    { id: number; userId: number; token: string; expiresAt: Date } | undefined
  >;
  ...
  getStoreAssignmentsByUserId(
    userId: number
  ): Promise<(StoreAssignment & { store: Store })[]>;
  getStoreAssignmentsByStoreId(
    storeId: number
  ): Promise<(StoreAssignment & { user: User })[]>;
  getActiveStoreAssignments(): Promise<
    (StoreAssignment & { user: User; store: Store })[]
  >;
  getStoreAssignmentsByUserAndDate(
    userId: number,
    date: Date
  ): Promise<StoreAssignment[]>;
  createStoreAssignment(
    assignment: InsertStoreAssignment
  ): Promise<StoreAssignment>;
  updateStoreAssignment(
    id: number,
    assignment: Partial<InsertStoreAssignment>
  ): Promise<StoreAssignment | undefined>;
  ...
  getWorkItemById(
    ...
    (WorkItem & { user?: User; store?: Store; creator?: User }) | undefined
  >;
  getAllWorkItems(): Promise<(WorkItem & { user?: User; store?: Store })[]>;
  getWorkItemsByUserId(
    userId: number
  ): Promise<(WorkItem & { store: Store })[]>;
  getWorkItemsByStoreId(
    storeId: number
  ): Promise<(WorkItem & { user: User })[]>;
  ...
  updateWorkItem(
    id: number,
    workItem: Partial<InsertWorkItem>
  ...
  getInventoryByStoreId(
    storeId: number
  ): Promise<(Inventory & { product: Product; shelf: Shelf })[]>;
  getLowStockItems(): Promise<
    (Inventory & { product: Product; shelf: Shelf; store: Store })[]
  >;
  ...
  getRecentActivities(
    limit: number
  ): Promise<(Activity & { product: Product; user: User; store: Store })[]>;
  ...
  getStockTakeWithItems(
    ...
    | (StockTake & { items: (StockTakeItem & { product: Product })[] })
    | undefined
  >;
  createStockTake(stockTake: InsertStockTake): Promise<StockTake>;
  updateStockTake(
    id: number,
    data: Partial<StockTake>,
    editorId: number,
    auditComment: string
  ): Promise<StockTake>;
  updateStockTakeItems(
    stockTakeId: number,
    items: InsertStockTakeItem[]
...
export class MemStorage implements IStorage {
  ...
  async getMerchandisingDataByWorkItemId(
    workItemId: number
  ): Promise<any | null> {
    return null;
  }

  async getCompetitorMerchandisingByWorkItemId(
    workItemId: number
  ): Promise<any | null> {
    return null;
  }
  ...
  private passwordResetTokens: Map<
    number,
    { id: number; userId: number; token: string; expiresAt: Date }
  >;
  ...
  async getActiveAlerts(): Promise<
    (Alert & { product: Product; store: Store })[]
  > {
  ...
  async getStoreAssignmentsByUserId(
    userId: number
  ): Promise<(StoreAssignment & { store: Store })[]> {
  ...
  async getStoreAssignmentsByStoreId(
    storeId: number
  ): Promise<(StoreAssignment & { user: User })[]> {
    const assignments = Array.from(this.storeAssignments.values()).filter(
  ...
  async getActiveStoreAssignments(): Promise<
    (StoreAssignment & { user: User; store: Store })[]
  > {
    const now = new Date();
    const assignments = Array.from(this.storeAssignments.values()).filter(
  ...
  async getActiveWorkItems(): Promise<
    (WorkItem & { user: User; store: Store })[]
  > {
    const items = Array.from(this.workItems.values()).filter(
  ...
  async createMerchandisingData(data: {
    ...
    try {
      ...
      const promotionResult = await db
        .insert(merchandisingPromotions)
        .values({
      ...
      console.log(
        `Added ${savedItems.length} merchandising items to the database`
      );
      ...
      if (workItem && workItem.status === WorkItemStatus.PENDING) {
        await this.updateWorkItem(data.workItemId, {
      ...
      await this.createActivity({
        ...
        storeId: data.storeId,
        productId:
          data.merchandisingItems.length > 0
            ? data.merchandisingItems[0].productId
            : 0,
  ...
  async createCompetitorMerchandising(data: {
    ...
    try {
      ...
      const result = await db
        .insert(competitorMerchandising)
        .values(insertValues)
        .returning();
      ...
      console.log(
        "Saved competitor merchandising data to database:",
        competitorData
      );
    ...
    try {
      ...
      if (data.products && data.products.length > 0) {
        for (const product of data.products) {
          ...
          `,
            [order.id, product.productId, product.quantity, null]
          );
        }
...
export class DatabaseStorage implements IStorage {
  ...
  async createCompetitorMerchandising(data: any): Promise<any> {
    ...
    try {
      ...
      if (data.workItemId) {
        console.log(
          `Associating competitor merchandising data with work item ID: ${data.workItemId}`
        );
        ...
        try {
          ...
          console.log(
            "Could not add workItemId to competitor data, it might not be in the schema yet"
          );
        }
      }
  ...
  async getStockTakeByWorkItemId(workItemId: number): Promise<any | null> {
    try {
      ...
      console.log(
        `Fetching stock take data for work item ${workItemId} (store: ${workItem.storeId}, user: ${workItem.userId})`
      );
      ...
      console.log(
        `Found ${stockTakesWithWorkItemId.length} potential stock takes for work item ${workItemId}`
      );
      ...
      if (workItem.completedAt) {
        ...
        if (matchingStockTake) {
          console.log(
            `Found matching stock take ${matchingStockTake.id} by completion time`
          );
      ...
      if (completedStockTakes.length > 0) {
        console.log(
          `Found most recent stock take ${completedStockTakes[0].id} for work item ${workItemId}`
        );
  ...
  async getOrderByWorkItemId(workItemId: number): Promise<any | null> {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Error in direct SQL query for orders, falling back to original schema query:",
          sqlError
        );
        ...
        if (existingOrder && existingOrder.length > 0) {
          ...
          console.log(
            `Found existing order for work item ${workItemId} using fallback query`
          );
          return order;
        }
      }
      ...
      if (orderResult && orderResult.rows.length > 0) {
        console.log(
          `Found existing order for work item ${workItemId} using direct SQL`
        );
        ...
        if (orderData.pictures) {
          console.log(
            "Processing order pictures, original format:",
            typeof orderData.pictures
          );
          try {
            ...
            if (typeof orderData.pictures === "string") {
              try {
                ...
                orderData.pictures = Array.isArray(parsed)
                  ? parsed
                  : [orderData.pictures];
              } catch (e) {
            ...
            // If pictures is already an object but not an array, convert to array
              ...
              typeof orderData.pictures === "object" &&
              !Array.isArray(orderData.pictures)
            ) {
        ...
        try {
          ...
          try {
            console.log(
              "Attempting to retrieve product data even without order items"
            );
            ...
            console.log(
              "Fallback product retrieval also failed:",
              fallbackError
            );
            orderData.items = [];
          }
      ...
      if (stockTake && stockTake.items && stockTake.items.length > 0) {
        ...
        const lowStockItems = stockTake.items.filter(
          (item) =>
            item.quantity === 0 ||
        ...
        if (lowStockItems.length > 0) {
          console.log(
            `Found ${lowStockItems.length} items for potential order from work item ${workItemId}`
          );
          ...
          const orderItems = lowStockItems.map((item) => ({
            ...
            quantity: item.product?.minStockLevel
              ? Math.max(item.product.minStockLevel - item.quantity, 1)
              : 1,
        ...
        console.log(
          `No stock take found or no items in stock take for work item ${workItemId}`
        );
      }
  ...
  async getMerchandisingDataByWorkItemId(
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Error in direct SQL query for merchandising (likely schema mismatch), falling back:",
          sqlError
        );
      ...
      if (
        ...
        typeof merchandisingData.promotionPictures === "string"
      ) {
        try {
          // Try to parse as JSON if it's a JSON string
          merchandisingData.promotionPictures = JSON.parse(
            merchandisingData.promotionPictures
          );
        } catch (e) {
          // If parsing fails, ensure it's an array
      ...
      try {
        ...
        console.log(
          "Error fetching merchandising items (may not exist in schema):",
          itemsError
        );
        merchandisingData.items = [];
      }
      ...
      console.log(
        `Found merchandising data for work item ${workItemId}:`,
        merchandisingData
      );
      return merchandisingData;
    } catch (error) {
  ...
  async getCompetitorMerchandisingByWorkItemId(
    ...
    console.log(
      "Getting competitor merchandising by work item ID:",
      workItemId
    );
    try {
      // First, get the work item to find store and user IDs
      ...
      try {
        ...
        // fall back to the original approach
        console.log(
          "Error in direct SQL query for competitor data (likely schema mismatch), falling back:",
          sqlError
        );
      ...
      if (result.rows.length > 0) {
        ...
        if (
          ...
          typeof competitorData.promotionPictures === "string"
        ) {
          try {
            // Try to parse as JSON if it's a JSON string
            competitorData.promotionPictures = JSON.parse(
              competitorData.promotionPictures
            );
          } catch (e) {
            // If parsing fails, ensure it's an array
        ...
        console.log(
          `Found competitor data for work item ${workItemId}:`,
          competitorData
        );
        return competitorData;
      } else {
      ...
      console.error(
        "Error getting competitor merchandising by work item ID:",
        error
      );
      return null;
    }
  ...
  async getRecentlyOrderedProducts(
    storeId: number,
    limit: number = 5
  ): Promise<Product[]> {
    try {
  ...
  async getProductsByCategoryLimit(
    category: string,
    limit: number = 5
  ): Promise<Product[]> {
    try {
  ...
  async getStockTakesByUserId(userId: number): Promise<StockTake[]> {
    return await db
      .select()
      .from(stockTakes)
      .where(eq(stockTakes.userId, userId));
  }

  async getStockTakeWithItems(
    id: number
  ): Promise<
    | (StockTake & { items: (StockTakeItem & { product: Product })[] })
    | undefined
  > {
    console.log(`DatabaseStorage.getStockTakeWithItems called with ID: ${id}`);
    ...
    const [stockTake] = await db
      ...
      .where(eq(stockTakes.id, id));
    if (!stockTake) {
      console.log(`No stock take found with ID: ${id}`);
      return undefined;
    }

    console.log(`Raw stock take from database:`, {
      id: stockTake.id,
    ...
    const result = { ...stockTake, items };
    console.log(`Final stock take result:`, {
      id: result.id,
      pictures: result.pictures,
      picturesLength: result.pictures?.length,
    });
  ...
  async createStockTake(stockTake: InsertStockTake): Promise<StockTake> {
    ...
    picturesToSave = picturesToSave.filter(
  ...
  async getStockTakeItemsByStockTakeId(
    stockTakeId: number
  ): Promise<StockTakeItem[]> {
    return await db
      .select()
  ...
  async getAssignmentByWorkItemId(
    workItemId: number
  ): Promise<StoreAssignment | undefined> {
    const result = await db
      .select({
      ...
      .innerJoin(
        workItems,
        eq(workItems.storeAssignmentId, storeAssignments.id)
      )
      .where(eq(workItems.id, workItemId));
  ...
  async getStoreAssignmentsByUserAndDate(
    userId: number,
    date: Date
  ): Promise<StoreAssignment[]> {
  ...
  > {
    try {
      ...
      console.log(
        `Completed work items count: ${completedCount.rows[0].count}`
      );

      console.log(
        "Running direct SQL query for ALL work items, including completed ones"
      );
      ...
      console.log(
        `Direct SQL query returned ${directQueryResult.rows.length} work items`
      );

      if (directQueryResult.rows.length > 0) {
        console.log(
          "First 3 items from direct query:",
  ...
  async deleteStore(id: number): Promise<boolean> {
    try {
      ...
      const storeTakes = await db
        .select()
        .from(stockTakes)
        .where(eq(stockTakes.storeId, id));
      for (const take of storeTakes) {
        await db
          .delete(stockTakeItems)
          .where(eq(stockTakeItems.stockTakeId, take.id));
      }
      await db.delete(stockTakes).where(eq(stockTakes.storeId, id));

      // Get all shelves in this store to delete related inventory
      const storeShelves = await db
        .select()
        .from(shelves)
        .where(eq(shelves.storeId, id));
  ...
  async getInventoryByStoreId(
    ...
    const storeShelvesResult = await db
      .select()
      .from(shelves)
      .where(eq(shelves.storeId, storeId));
  ...
  async adjustInventory(
    ...
    const [existingInventory] = await db
      ...
      .from(inventory)
      .where(
        and(eq(inventory.productId, productId), eq(inventory.shelfId, shelfId))
      );
  ...
  async getAllActivitiesWithRelations(
    limit: number = 10,
    offset: number = 0
  ): Promise<(Activity & { product: Product; user: User; store: Store })[]> {
    const result = await db
  ...
  async getStockTakeReportsData(timeframe: string): Promise<any> {
    ...
    const stockTakesResult = await db
      .select({
        total: count(),
        completed: count(stockTakes.id).filter(
          eq(stockTakes.status, "completed")
        ),
        pending: count(stockTakes.id).filter(eq(stockTakes.status, "pending")),
        canceled: count(stockTakes.id).filter(
          eq(stockTakes.status, "canceled")
        ),
      })
      .from(stockTakes)
      .where(
  ...
  async getOrderReportsData(timeframe: string): Promise<any> {
    ...
    try {
      ...
      const ordersResult = await db
        ...
        .from(orders)
        .where(
          and(gte(orders.createdAt, startDate), lte(orders.createdAt, today))
        );
