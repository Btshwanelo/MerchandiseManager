    ...
    <div className="container mx-auto p-4">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Badge variant={workItem.status === WorkItemStatus.COMPLETED ? "success" : "secondary"}>
            {workItem.status}
          </Badge>
        </div>
      </div>
          ...
          <CardContent className="grid gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm">{formatDate(workItem.createdAt)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {stockTake ? (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div className="text-center p-4 border rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">
                      {stockTake.items?.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0) || 0}
                    </div>
                ...
                {stockTake.items && stockTake.items.length > 0 && (
                  <>
                    ...
                    <h3 className="font-medium text-sm mb-2">Stock Take Items</h3>
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse border border-border">
                        <thead>
                          <tr className="bg-muted">
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Product</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">SKU</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Quantity</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Location</th>
                          </tr>
                        </thead>
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm mt-2">If this work item required inventory counting, the merchandiser has not completed it yet.</p>
              </div>
            )}
          </CardContent>
        </Card>
            ...
            {merchandising ? (
              <>
                {merchandising.items && merchandising.items.length > 0 && (
                  <>
                    <h3 className="font-medium text-sm mb-2">Merchandising Items</h3>
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse border border-border">
                        <thead>
                          <tr className="bg-muted">
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Product</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">SKU</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Price</th>
                          </tr>
                        </thead>
                        <tbody>
                          {merchandising.items.map((item: any, index: number) => (
                            <tr key={index} className={index % 2 === 0 ? "bg-background" : "bg-muted/30"}>
                              ...
                              <td className="px-4 py-2 text-sm">{item.productSku || "N/A"}</td>
                              <td className="px-4 py-2 text-sm">${(item.price / 100).toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {merchandising.promotionPictures && (
                  <>
                    <Separator className="my-4" />
                    <h3 className="font-medium text-sm mb-2">Merchandising Photos</h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {(() => {
                        const picturesToRender = processImagePaths(merchandising.promotionPictures);
              ...
              <>
                      ...
                      <table className="w-full border-collapse border border-border">
                        <thead>
                          <tr className="bg-muted">
                            ...
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">SKU</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Quantity</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Unit Price</th>
                            <th className="border border-border px-4 py-2 text-left text-sm font-medium">Total</th>
                          </tr>
                        </thead>
                        <tbody>
                          {order.items.map((item: any, index: number) => (
                            <tr key={index} className={index % 2 === 0 ? "bg-background" : "bg-muted/30"}>
                              ...
                              <td className="px-4 py-2 text-sm">{item.product?.sku || "N/A"}</td>
                              <td className="px-4 py-2 text-sm">{item.quantity}</td>
                              <td className="px-4 py-2 text-sm">${(item.product?.price / 100).toFixed(2)}</td>
                              <td className="px-4 py-2 text-sm">${((item.product?.price * item.quantity) / 100).toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {order.pictures && (
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {(() => {
                        const picturesToRender = processImagePaths(order.pictures);
