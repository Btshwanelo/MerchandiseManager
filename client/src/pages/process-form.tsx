...
type StockTakeSectionProps = {
  ...
  setLowStockItems?: (
    items: Array<{ product: Product; quantity: number; location: string }>
  ) => void;
...
const StockTakeSection = ({
  ...
  workItem,
}: StockTakeSectionProps) => {
  const [loading, setLoading] = useState(false);
  const [stockData, setStockData] = useState<
    { productId: number; quantity: number; location: string }[]
  >([]);
  ...
  const [shelfImages, setShelfImages] = useState<Array<File | string | null>>(
    Array(8).fill(null)
  );
      ...
      if (activeStep === "stock-take") {
        ...
        if (stockData.length > 0 || comments.trim() || pictures.length > 0) {
          ...
          toast({
            ...
            description:
              "Add some stock items, comments, or pictures before saving.",
            variant: "destructive",
          });
        }
      }
      ...
      if (isReadOnly && stockTake) {
        try {
          ...
          toast({
            ...
            description:
              "There was an error parsing the completed stock take data.",
            variant: "destructive",
          });
        }
      }
  ...
  const submitStockTake = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["/api/stores", storeId, "stock-takes"],
      });
      queryClient.invalidateQueries({
        queryKey: ["/api/stock-takes/by-work-item", workItemId],
      });
                ...
                <div className="flex">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700">
                      This stock take has been completed and cannot be modified.
                      Below is a summary of the recorded inventory.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {stockTakeType === "both" && (
                <div className="md:col-span-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: "shelf" | "back_store") =>
            ...
            {isReadOnly && (
              <div className="mb-4">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Below is a summary of all products counted during this stock
                  take.
                </p>
              </div>
            )}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            ...
            {isReadOnly && shelfImages.filter(Boolean).length > 0 && (
              <Badge variant="outline" className="bg-muted">
                {shelfImages.filter(Boolean).length}{" "}
                {shelfImages.filter(Boolean).length === 1 ? "image" : "images"}
              </Badge>
            )}
          </div>
            ...
            <div>
              {shelfImages.filter(Boolean).length === 0 ? (
                <div className="bg-muted/50 rounded-lg p-8 text-center">
                  ...
                  <p className="text-muted-foreground">
                    No shelf images were uploaded for this stock take.
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <div className="w-full h-full bg-muted flex items-center justify-center">
                        {image && typeof image.toString === "function" ? (
                          (() => {
                            const imageStr = image.toString();
                            return imageStr.includes(".jpg") ||
                              imageStr.includes(".png") ||
                              imageStr.includes(".jpeg") ? (
                            ...
                            <img 
                              ...
                              onError={(e) => {
                                ...
                                (e.target as HTMLImageElement).className =
                                  "w-1/2 h-1/2 object-contain opacity-50";
                              }}
                            />
                          ) : (
                            <span className="text-sm text-center px-2 text-muted-foreground">
              ...
              <div className="mt-4 border-t pt-4">
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-md p-4 mb-4">
                  ...
                  <p className="text-sm text-blue-600 mb-3">
                    Add images by selecting any empty slot above or use the
                    button below to add multiple images at once
                  </p>
                ...
                <p className="text-sm text-muted-foreground">
                  Upload photos of the shelf to document the stock take
                </p>
              </div>
            </div>
          )}
            ...
            <div>
              {!comments || comments.trim() === "" ? (
                <div className="bg-muted/50 rounded-lg p-6 text-center">
                  ...
                  <p className="text-muted-foreground">
                    No additional comments were provided for this stock take.
                  </p>
                </div>
              ) : (
      ...
      {isReadOnly && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
          ...
          <p className="text-sm text-green-700">
            This stock take has been submitted and completed. No further changes
            can be made.
          </p>
        </div>
      )}
                      ...
                      {stockTake?.items && stockTake.items.length > 0 && (
                        <div>
                          <h3 className="text-sm font-medium mb-2">
                            Inventory Items ({stockTake.items.length})
                          </h3>
                    ...
                    {!merchandisingData ? (
                      <div className="text-center py-12 px-4">
                        ...
                        <h3 className="text-lg font-medium text-muted-foreground mb-1">
                          No Merchandising Data
                        </h3>
                        <p className="text-sm text-muted-foreground max-w-md mx-auto">
                          This work item does not have any merchandising data
                          submitted by the merchandiser.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                  ...
                  <CardContent>
                    {!competitorData ? (
                      <div className="text-center py-8 text-muted-foreground">
                        <p>
                          No competitor analysis data available for this work
                          item.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p>
                              {competitorData.competitorName || "Not specified"}
                            </p>
                          </div>
                        </div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          {addedPromotionProducts.length === 0 ? (
            <div className="bg-muted/50 rounded-lg p-8 text-center">
              ...
              <p className="text-muted-foreground">
                Select a product and price to add it to the promotion.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
      ...
      >
        ...
        {promotionItems.length === 0 && promotionPictures.length === 0
          ? "Skip Merchandising Step"
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Failed to submit competitor analysis",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    return inventory
      ...
      .map((item) => {
        ...
        return {
          ...
          quantityToOrder: product
            ? Math.max(product.minStockLevel - item.quantity, 1)
            : 1,
        };
      });
  }, [products, inventory]);
    ...
    try {
      ...
      try {
        ...
        toast({
          title: "Error",
          description:
            "Failed to mark work item as completed, but your order was saved",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      <Button onClick={submitOrder} disabled={loading} className="w-full">
        ...
        {!notes && orderItems.length === 0
          ? "Skip Order Step & Complete"
...
const ProcessForm = () => {
  ...
  const [lowStockItems, setLowStockItems] = useState<
    Array<{ product: Product; quantity: number; location: string }>
  >([]);
    ...
    if (workItem && workItem.status === WorkItemStatus.PENDING) {
      try {
        ...
        try {
          ...
          console.log(
            "Error using main status update endpoint, trying fallback",
            err
          );
          // Fallback to the dedicated endpoint for updating status via assigned-work-items
          await apiRequest(
            "PUT",
            `/api/assigned-work-items/${workItemId}/status`,
            {
              status: WorkItemStatus.IN_PROGRESS,
            }
          );
      ...
      if (workItemId && !storeId) {
        try {
          ...
          const response = await fetch(
            `/api/assigned-work-items/${workItemId}`
          );
          if (response.ok) {
            const workItemData = await response.json();
            if (workItemData && workItemData.storeId) {
              console.log(
                `Found storeId ${workItemData.storeId} for workItemId ${workItemId}`
              );
              // Redirect with both parameters
              navigate(
                `/process-form?workItemId=${workItemId}&storeId=${workItemData.storeId}`
              );
              return;
            }
          } else {
            ...
            console.log(
              "First endpoint failed, trying admin endpoint as fallback"
            );
            ...
            if (adminResponse.ok) {
              ...
              if (adminWorkItemData && adminWorkItemData.storeId) {
                console.log(
                  `Found storeId ${adminWorkItemData.storeId} from admin endpoint for workItemId ${workItemId}`
                );
                // Redirect with both parameters
                navigate(
                  `/process-form?workItemId=${workItemId}&storeId=${adminWorkItemData.storeId}`
                );
                return;
              }
            }
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-8">
              ...
              <p className="mb-2">
                Unable to load work item or store information.
              </p>
              {workItemError && (
                <p className="text-sm text-muted-foreground mb-1">
                  Work item error: {(workItemError as Error).message}
                </p>
              )}
              {storeError && (
                <p className="text-sm text-muted-foreground mb-4">
          ...
          <div className="mb-6 p-4 bg-muted rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                >
                  {workItem.status
                    ? workItem.status.charAt(0).toUpperCase() +
                      workItem.status.slice(1).replace("_", " ")
                    : "Unknown"}
                </span>
              </p>
              {workItem.dueDate && (
          ...
          <div className="mb-8 hidden md:block">
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-sm text-muted-foreground">
                  Orders of stock that is depleted etc
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col items-center w-full">
              {activeStep === "stock-take" && (
                <>
                  ...
                  <p className="text-sm text-muted-foreground text-center">
                    Step 1 of 4: Stock taking at shelf or Store
                  </p>
                </>
              )}
              ...
              {activeStep === "merchandising" && (
                <>
                  ...
                  <p className="text-sm text-muted-foreground text-center">
                    Step 2 of 4: Promotions for any of our products
                  </p>
                </>
              )}
              ...
              {activeStep === "competitor-analysis" && (
                <>
                  <div className="h-2 w-full rounded-full mb-2 bg-[#7ccd57]"></div>
                  <h3 className="font-medium text-[#7ccd57]">
                    Competitor Promotions
                  </h3>
                  <p className="text-sm text-muted-foreground text-center">
                    Step 3 of 4: Any promotions from competitors
                  </p>
                </>
              )}
              ...
              {activeStep === "order-placement" && (
                <>
                  ...
                  <p className="text-sm text-muted-foreground text-center">
                    Step 4 of 4: Orders of stock that is depleted
                  </p>
                </>
              )}
              ...
              {(activeStep === "completed" ||
                ...
                <div className="text-center py-10">
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 mx-auto max-w-lg text-left">
                        <div className="flex">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-green-700">
                              You are viewing a completed work item in read-only
                              mode.
                  ...
                  <p className="text-lg mb-6 max-w-md mx-auto">
                    You've successfully completed all the required steps for
                    this work item at {store.name}.
                  </p>
                  <Button size="lg" onClick={() => navigate("/my-assignments")}>
                    Return to My Assignments
                  </Button>
                </div>
              )}
          ...
          <div className="py-2">
            ...
            <div className="max-h-[200px] overflow-y-auto border rounded-md p-2">
              {lowStockItems.map((item, index) => (
                <div
                  key={`${item.product.id}-${item.location}-${index}`}
                  className="flex justify-between items-center p-2 border-b last:border-b-0"
                >
                  <div>
                    <p className="font-medium">{item.product.name}</p>
                    <p className="text-sm text-muted-foreground">
                      {item.location === "shelf" ? "Shelf" : "Back Store"} -
                      Current:{" "}
